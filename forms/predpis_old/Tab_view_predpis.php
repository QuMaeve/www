...
$add_obj_tab=mysql_query ('select distinct id_obj from predpis_old_obj_id'.$user_id.'_user order by id_obj');
					...
					if ($row_obj_tab[0] == ""){}
								...
								{//3
										...
										$flag_tab2=mysql_query ('select sum(flag) 
																	 from predpis_old_obj_id'.$user_id.'_user
						 ...
						 if	($r_flag2[0]>0){$text_buf=' checked '; $count_check_v++;}else{$text_buf='';}
										$value_obj='<p><input  type="checkbox" name="in_obj_tab"  										'.$text_buf.' value="'.	$row_obj_name[0].'" onclick="checkPredpisOLD('."'".$row_obj_name[0]."'".','."'".'iot'.$ii."'".') " id="iot'.$ii.'" />
						 ...
						 if	($r_flag1[0]>0){$text_buf=' checked '; $count_check_v++;}else{$text_buf='';}			
																...
																$value_adr[$c_a]= '<p><input  type="checkbox"
																 ...
																 onclick="checkPredpisOLD('."'".$r_a_tab[5]."'".','."'".'iat'.$iii."'".')"
						...
						while ($r_v_a_tab = mysql_fetch_row($v_a_tab))
						 ...
						 if	($r_flag[0]==1){$text_buf=' checked '; $count_check_v++;}else{$text_buf='';}
										...
										$d_tab=mysql_query ('SELECT  `date_predpis`
							   FROM predpis_old_obj_id'.$user_id.'_user
								 where id_obj="'.$row_obj_tab[0].'" and id_address="'.$r_a_tab[5].'" and id_v="'.$row_v_tab[0].'" ');
								 
							if($r_a_tab[3]==""){$n1="n";}else{$n1=$r_a_tab[3];}
							if($r_a_tab[4]==""){$n2="n";}else{$n2=$r_a_tab[4];}							
							while ($r_d = mysql_fetch_row($d_tab))
							{$date_predpis=' <input id="v_date_'.$r_flag[1].'" type="date" class="viol_end_date" value="'.$r_d[0].'"   data-fl_param="'.$r_flag[1].'" onkeyup=" rec_date_p(event)"/>';}	
								
									$value_viol=$value_viol. '<p><input  type="checkbox" name="in_v_tab"
								 '.$text_buf.'
								  value="'.	$row_v_tab[0].'" id="ivt'.$iiii.'"
								   onclick="checkPredpisOLD('."'".$r_flag[1]."'".','."'".'ivt'.$iiii."'".')"
								  />'.iconv("utf-8","windows-1251",$row_v_tab[1]).$date_predpis.' </p>';
									 $t_viol=$row_v_tab[2];
...
$buf='<table width="100%" border="2" name="Tab_form_predpis_old_head">
