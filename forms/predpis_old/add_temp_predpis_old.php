...
include_once("..\link\link.php");if (!mysql_query('SELECT * FROM predpis_old_obj_id'.$user_id.'_user')){
			mysql_query('create table IF NOT EXISTS predpis_old_obj_id'.$user_id.'_user (
		...
		from predpis_old_obj_id'.$user_id.'_user  where `id_obj` =  "'.$obj.'" and `id_city`=  "'.$city.'
		 ...
		 // echo $f_id_a.' '. $q_f_id_a.$id_val_ad. $id_val_ad[0].' val2';
			...
			if ( $id_ad==0){ $id_ad=1;}
				...
				$insert_tab=mysql_query ('INSERT INTO  
					`predpis_old_obj_id'.$user_id.'_user` (
...
include_once("Tab_view_predpis.php");
