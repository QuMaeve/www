...
while ($r1 = mysql_fetch_row($q1))
 {
if($otdel==""){$otdel='"'.$r1[0].'"';}else{$otdel=$otdel.',"'.$r1[0].'"';}
...
while ($r_otdel1 = mysql_fetch_row($q_otdel1))
 {
if($otdel2==""){$otdel2='"'.$r_otdel1[0].'"';}else{$otdel2=$otdel2.',"'.$r_otdel1[0].'"';}
...
if 	($otdel2==""){$id_tab="";}else{$id_tab=" and `id` in(".$otdel2.")";}      
...
while ($r1 = mysql_fetch_row($q1))
 {
if ($prot_id=="") {$prot_id=$r1[0];}else{$prot_id=$prot_id.', '.$r1[0];}
  }
  if ($prot_id=="") { $tab=$tab.'<td height="115" width="46">0</td>';}else{
