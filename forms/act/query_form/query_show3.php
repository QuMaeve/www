...
$text_q='SELECT  `act`.`num` ,  `act`.`date_act` ,  `act`.`address_id` ,  `act`.`obj_id` ,    `linc_act_violation`.`id_v` ,  `link_act_workes`.`id_user` ,  `order`.`num_order` ,  `order`.`date_order`, `act`.`id` 
FROM act
LEFT JOIN  `linc_act_violation` ON  `act`.`id` =  `linc_act_violation`.`id_act` 
...
$v_act="";
$t_q='SELECT  `violation`.`NAME_CODE` ,  `type_violation`.`Name_type` ,  `name_obj_violation`.`name_obj` FROM violation
