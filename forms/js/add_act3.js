...
function admin_choice() {
    ...
    switch (val)
    {
        case ("1"):
            document.getElementById("v_control").style.display="block";
            document.getElementById("d_control").style.display="none";
            document.getElementById("prover_ob_sobr").style.display="none";
            break;
        case ("2"):
            document.getElementById("prover_ob_sobr").style.display="block";
            document.getElementById("v_control").style.display="none";
            document.getElementById("d_control").style.display="block";
...
function add_act() {
    ...
    if (sel_d==2){
        var pr=document.getElementById('prover_ob_sobr_check');
        if(pr.checked==true){
            var rad=document.getElementsByName('l_obj');
            for (var i=0;i<rad.length; i++) {
                if (rad[i].checked) {
                    var obj_rasp=rad[i].value;
                    if(obj_rasp===""){alert("Указанны не все данные!!!");}
                    else {
                        var city = document.getElementById('l_city').value;
                        var street = document.getElementById('l_street').value;
                        ...
                        $.ajax({
                            type: 'post',
                            url: "forms/act/add_address.php",
                            data: {
                                city: city, street: street, house: house, korpus: korpus
                            ...
                            success: function (result4) {
                                ...
                                if (result4 == "") {
                                 ...
                                 //   alert(u_id + " " + depart + " " + choice_list + " " + result4 + " " + obj_rasp + " " + date_p);
                                    add_add_act(u_id, choice_list, sel_d, result4, obj_rasp, date_p);
            ...
            if(obj_rasp===""){alert("Указанны не все данные!!!");}
               ...
               // alert(u_id + " " + " " + choice_list + " " + address_rasp + " " + obj_rasp + " " + date_p);
                add_add_act(u_id, choice_list, sel_d, address_rasp, obj_rasp, date_p);
            }

        }
    }
    else {    //  alert(u_id+" "+" "+choice_list+" "+date_p);
...
function prover_ob_sobr_check_fun() {
    // alert("мышь");
    var pr=document.getElementById('prover_ob_sobr_check');
    if(pr.checked==true){//alert("галочка");}
        document.getElementById("prover_ob_sobr_data").style.display="block";
        document.getElementById("choice").style.display="none";
    }

    else
    {document.getElementById("prover_ob_sobr_data").style.display="none";
...
function add_add_act(u_id,choice_list,depart,address_rasp,obj_rasp,date_p) {
//alert(obj_rasp+' '+address_rasp);
     if(address_rasp===""){alert("Указанны не все данные!!!");}
      else{
         ...
         if (document.getElementById("v_control").style.display == "block") {
             ...
             if (document.getElementById("risc_p").checked) {
                 if (risc == 1) {
                     ...
                     var risc_p = document.getElementById("risc_v_d").value;
                 }
             } else {
                 var risc_p = 0;
             }
                     ...
                     case ("1"):
                         ...
                         if (pl_obj == "") {
                             ...
                             var val_data = {
                                 ...
                                 date_p: date_p,
                                 rasp_id: choice_list,
                                 address_id: address_rasp
                                 ,
                                 obj_id: obj_rasp,
                                 num_time: time_hour_or_day,
                                 area: pl_obj,
                                 risc: risc
                                 ,
                                 risc_v: risc_p,
                     ...
                     case ("2"):
                         if (Size_n == "") {
                             ...
                             var val_data = {
                                 ...
                                 obj_id: obj_rasp,
                                 num_time: time_hour_or_day,
                                 recalculation_charge: recalculation_charge
                                 ,
                                 size_recalculation: Size_n,
                                 in_police: in_police,
                                 v_li: v_li,
                                 predpis: predpis,
                                 vv: vv
