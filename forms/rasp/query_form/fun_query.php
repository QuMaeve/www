			 ...
			 $q_obj=mysql_query ("SELECT Name_org, id FROM complaints_obj ");
				while ($r_obj = mysql_fetch_row($q_obj)) 
					{ if ($r_obj[0] == ""){}
						...
						//echo '<option value="'.$r_incom[1].'"> '.$r_incom[0].'</option>';
								$op3=$op3.'<option value="'.$r_obj[1].'"> '.iconv("utf-8","windows-1251",$r_obj[0]).'</option>';
							}
					}
					$op3=$op3.'</select>';
					echo $op3;
...
function showBasis245($text){
include_once('../../general/place_form.php');

}

function list_city_obr($s_city) {
echo $s_city;
}
